...
| 4 | **Locator Map** (`Locator-mapper.-SKILL.md`) | An agentic 3-step workflow that maps all interactive elements on a webpage to unique CSS selectors and saves them as a local JSON file. Uses a two-layer architecture: `extract.js` (Playwright) greedily fetches raw element attributes, then the AI names elements and picks stable selectors. Works with Playwright, Cypress, Selenium, and Puppeteer. | _"Map page elements for …"_ · _"Generate locators for test automation"_ · _"Get me the locators for this page"_ · _"Extract element identifiers from a URL"_ · _"Scan a webpage for interactive elements"_ · _"Build a selector reference"_ |
